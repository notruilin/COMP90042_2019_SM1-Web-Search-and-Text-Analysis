...
with open("ir_dev_output.json") as ir_train:
    ir_set = json.load(ir_train)
...
with open("devset.json") as train_json:
    ...
    for data in train_set.items():
        ...
        if label == "NOT ENOUGH INFO":
            evidences = ir_set[data[0]]["evidence"]
        for evidence in evidences:
            ...
            data_one = {}
            data_one["claim"] = process_string(claim)
            data_one["evidence"] = process_string(text)
            if label:
...
with open("dev_evidence.jsonl", mode='a') as out_file:
