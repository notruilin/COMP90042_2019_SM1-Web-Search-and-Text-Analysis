...
def get_synonyms(term, vocabulary, primary_sense):
    ...
    for synonym in vocabulary:
        if synonym != term and primary_sense[term] and primary_sense[synonym] and primary_sense[term].wup_similarity(primary_sense[synonym]) and primary_sense[term].wup_similarity(primary_sense[synonym]) >= 0.8:
...
def extract_vocabulary(related_sentences, claim):
    vocabulary = set()
    for doc in related_sentences:
        vocabulary = vocabulary.union(extract_terms_from_doc(doc[2]))
    vocabulary = vocabulary.union(extract_terms_from_doc(claim))
    return vocabulary

def extract_sentences_as_docs(related_sentences):
    docs = {}
    for doc in related_sentences:
        docs[(doc[0],doc[1])] = extract_terms_from_doc(doc[2])
    return docs
...
def query_topk_sentences(query, inverted_index, k, vocabulary, primary_sense):
    ...
    for term in query:
        ...
        for key, w in posting_list:
            accumulators[key] += w
    results = accumulators.most_common(k)
    if len(results) == 0:
        return []
    max_score = results[0][1]
    top_sentences = []
    for sentence in results:
        if max_score - sentence[1] > 0.15 or sentence[1] <= 0.1:
            break
