...
def extract_all_NN(tokens_tags):
    NNs = []
    for i in range(len(tokens_tags)):
        if tokens_tags[i][0].isalnum() and tokens_tags[i][1].startswith("NN"):
            NNs.append(tokens_tags[i][0].lower())
    query = " ".join(NNs)
    return query
...
def add_to_related_sentences(hit):
    related_sentences.append((hit["title"], hit["sen_id"], hit["text"]))

def sort_sentences(claim):
    vocabulary = SortSens.extract_vocabulary(related_sentences, claim)
    primary_sense = SortSens.get_primary_sense(vocabulary)
    docs = SortSens.extract_sentences_as_docs(related_sentences)
...
for data in train_set.items():
    ...
    last_hit = get_related_docs(escape_query(extract_last_NN(tokens_tags)), "title", 2)
    NN_hits = get_related_docs(process_query(extract_all_NN(tokens_tags)), "text", 2)
    claim_hits = get_related_docs(process_query(claim), "text", 2)
    related_sentences = []
    
    for hits in [title_hit, last_hit, NN_hits, claim_hits]:
        if hits != None:
            for hit in hits:
                add_to_related_sentences(hit)
